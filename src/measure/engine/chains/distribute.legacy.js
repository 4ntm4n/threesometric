...
function writeNodePositionsEvenOrLocked(graph, solNodes, orderedNodes, orderedEdges, locked, freeWithVals, total) {
  ...
  if (!startM?.known) {
    ...
    if (!coinc) {
      // Kolla även om slutänden matchar nån känd nod – i så fall vänd och använd den som start
  ...
  if (bothKnownNow) {
    ...
    // Ankra start i första hand på känd metric via world-coincidens, annars world→metric (0,0,0)
    origin = startM?.known
      ? startM
    ...
    // Skriv in startens origin om den inte redan var känd
  ...
  for (let i=0; i<orderedEdges.length; i++) {
    ...
    if (!isFiniteNumber(len)) {
      // fallback: aktuell sol/world-längd (säkrar bort NaN)
...
// Försök rotera så att nodes börjar på endA, annars endB (best effort)
...
// Se till att ordningen följer kedjans faktiska världsriktning
function orientOrderToWorldDirection(graph, nodes, edges) {
  ...
  if (refLen <= 1e-9) return; // kan inte avgöra

  // chord enligt world mellan nuvarande start->end
  ...
  // Om chord går “baklänges” mot referens → vänd listorna
...
// Hitta en känd metric-nod i solution som har (nästan) samma world-position
