...
export function createDrawManager({ THREE: THREE_NS, scene, camera, renderer3D, controls, overlay, picker, snapper, modelGroup, permanentVertices }) {
  state.camera = camera;
  ...
  function onMouseMove(e, pickPlaneMesh) {
    if (document.pointerLockElement) {
      if (state.draw.isDrawing && state.draw.hasStart) {
        overlay.setVirtualCursorTo2D({ x: overlay.virtualCursorPix.x + e.movementX, y: overlay.virtualCursorPix.y + e.movementY });
      }
    } else {
  ...
  function commitIfAny() {
    ...
    if (worldLen > 1e-6) {
      ...
      const line = new THREE.Line(geom, new THREE.LineBasicMaterial({ color: 0xffffff, depthTest:false, depthWrite:false }));
      line.renderOrder = 1;
      modelGroup.add(line);
      ...
      picker.pickables.add(pickCyl);

      state.draw.lineStartPoint.copy(end3D);

      if (!isPointVisible(end3D)) {
        const target = end3D.clone();
        const pos = target.clone().add(state.iso.ISO_OFFSET);
        controls.setLookAt(pos.x, pos.y, pos.z, target.x, target.y, target.z, true).then(() => {
          if (state.draw.isDrawing) overlay.recenterCursorToStart();
        });
      } else {
  ...
  function onKeyUp(e, { resetIsoAndFitAll }) {
    if (e.code === 'Escape') {
      state.draw.isDrawing = false; state.draw.hasStart = false; state.draw.pending = false;
  ...
  function onPointerLockChange() {
    if (!document.pointerLockElement) {
      ...
      if (state.draw.hasStart) {
        ...
        overlay.recenterCursorToStart();
        if (state.draw.pending) { state.draw.isDrawing = true; state.draw.pending = false; }
      }
    }
  }
